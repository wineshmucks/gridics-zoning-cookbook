...
def test_query_customer_zoning_code_ignores_address_like_client_id(monkeypatch) -> None:
    tenant = DummyTenant()
    captured: dict[str, object] = {}
    ...
    result = query_customer_zoning_code(
        query="What are the height limits?",
        limit=4,
        client_id="3148 Mary St, Miami, FL 33133",
        run_context=DummyRunContext({"client_id": "springfield"}),
    )
...
def test_standardize_address_marks_exact_matches_as_not_needing_confirmation() -> None:
    result = standardize_address("3148 Mary St, Miami, FL 33133")

    assert result == {
        "input_address": "3148 Mary St, Miami, FL 33133",
...
def test_standardize_address_marks_normalization_changes_as_needing_confirmation() -> None:
    result = standardize_address("3148 Mary St Apt 2, Miami, FL 33133")

    assert result == {
        "input_address": "3148 Mary St Apt 2, Miami, FL 33133",
        "standardized_address": "3148 Mary St, Miami, FL 33133",
...
def test_analyze_customer_zoning_request_falls_back_when_query_is_missing(monkeypatch) -> None:
    captured: dict[str, object] = {}
...
def test_analyze_customer_zoning_request_reuses_recent_standardized_address(monkeypatch) -> None:
    ...
    standardize_address("3148 Mary St, Miami, FL 33133", run_context=run_context)
...
def test_analyze_customer_zoning_request_reuses_recent_standardized_address_from_session_state_kwarg(
    ...
    standardize_address("3148 Mary St, Miami, FL 33133", session_state=run_context.session_state)
...
def test_analyze_customer_zoning_request_reuses_active_property_for_story_follow_up(monkeypatch) -> None:
    gridics_calls: list[dict[str, str]] = []
    knowledge_calls: list[str] = []
    run_context = DummyRunContext({"client_id": "springfield"})
    ...
    def fake_extract_gridics_zoning_summary(payload):
        assert payload == {"mock": "payload"}
        return {
            "resolved_address": "614 S Miami Ave",
            "resolved_city": "Miami",
            "resolved_state": "FL",
            "zone_combination_name": "T6-48B-O",
            "typology": "Mixed Use",
    ...
    def fake_query_customer_zoning_code(*, query: str, limit: int, client_id: str, run_context=None):
        knowledge_calls.append(query)
        return {"query": query, "results": [{"name": "Height standard", "content": "Height rules."}]}
    ...
    result = analyze_customer_zoning_request(
        query="how many stories is that?",
        run_context=run_context,
    )

    assert result["question_type"] == "specific_address"
    assert result["address_context"]["address_source"] == "session"
    assert result["address_context"]["detected_address"] == "614 S Miami Ave, Miami, FL 33130"
    assert result["follow_up_context"]["active_location"]["story_equivalent"] == 4
...
def test_analyze_customer_zoning_request_exact_address_skips_confirmation(monkeypatch) -> None:
...
def test_analyze_customer_zoning_request_exact_standardized_address_skips_gridics_mismatch_confirmation(monkeypatch) -> None:
    gridics_calls: list[dict[str, str | None]] = []
    knowledge_calls: list[dict[str, object]] = []
    ...
    monkeypatch.setattr(
        "app.agents.tools._extract_gridics_zoning_summary",
        lambda payload: {
            "resolved_address": "3675 S MIAMI AVE",
            "resolved_city": "Miami",
            "resolved_state": "FL",
            "zone_combination_name": "CI",
            "typology": "Civic",
