...
def list_public_customers(db: Session = Depends(get_db)) -> list[dict]:
    gridics_org_id = settings.gridics_clerk_organization_id.strip().lower() if settings.gridics_clerk_organization_id else None
    ...
    for customer in customers:
        customer_org_id = customer.clerk_organization_id.strip().lower() if customer.clerk_organization_id else None
        if customer.client_id.strip().lower() == "gridics":
            continue
        if gridics_org_id and customer_org_id == gridics_org_id:
            continue

        results.append(
            {
                "orgid": customer.clerk_organization_id or customer.client_id,
