...
class TenantPublicConfig:
    ...
    @property
    def market_served(self) -> str | None:
        return self.market
...
_CUSTOMER_ZONING_ASSISTANT_TARGET_ID = "customer_zoning_team"
_LEGACY_CUSTOMER_ZONING_ASSISTANT_TARGET_ID = "customer-zoning-agent"
_PUBLIC_CUSTOMER_ZONING_ASSISTANT_TARGET_ID = "customer-zoning-team"

ASSISTANT_TARGET_ID_ALIASES = {
    _LEGACY_CUSTOMER_ZONING_ASSISTANT_TARGET_ID: _CUSTOMER_ZONING_ASSISTANT_TARGET_ID,
    _PUBLIC_CUSTOMER_ZONING_ASSISTANT_TARGET_ID: _CUSTOMER_ZONING_ASSISTANT_TARGET_ID,
}
...
def _normalize_assistant_provider_keys(value: object) -> dict[str, str | None]:
    ...
    normalized: dict[str, str | None] = {"gemini": None}
    raw = value.get("gemini")
    if isinstance(raw, str):
        stripped = raw.strip()
        normalized["gemini"] = stripped or None
    return normalized
...
def _cache_keys_for_tenant_lookup(
    *,
    host: str | None = None,
    ...
    keys: list[str] = []
    if path_alias:
        keys.append(f"alias:{path_alias.lower()}")
    if client_id:
        keys.append(f"client:{client_id.lower()}")
    if jurisdiction_id:
        keys.append(f"jurisdiction:{jurisdiction_id.lower()}")
    if organization_id:
        keys.append(f"org:{organization_id.lower()}")
    if host:
        keys.append(f"host:{host.lower()}")
    if not keys:
        keys.append("default")
    return keys
...
def merge_assistant_provider_keys(
    ...
    merged = {"gemini": baseline.get("gemini")}
    if overrides.get("gemini"):
        merged["gemini"] = overrides["gemini"]
    return merged
...
def merge_tenant_experience_settings(
    ...
    *,
    ...
    if assistant_provider_keys is not None:
        ...
        if normalized_provider_keys.get("gemini"):
...
def resolve_tenant_public_config(
    ...
    *,
    ...
    normalized_jurisdiction_id = jurisdiction_id.strip().lower() if jurisdiction_id else None
    normalized_path_alias = normalize_tenant_path_alias(path_alias) if path_alias else None
    cache = get_cache_service()
    for key in _cache_keys_for_tenant_lookup(
        ...
        cached = cache.get_json(f"tenant-public:{key}")
        if cached is cache.cache_miss:
            continue
        if cached is None:
            return None
        try:
            ...
            cache.delete(f"tenant-public:{key}")
    ...
    elif normalized_jurisdiction_id:
        client = db.scalar(
            select(TenantClient).where(
                TenantClient.jurisdiction_id == normalized_jurisdiction_id,
                TenantClient.is_active.is_(True),
            )
        )
    ...
    cached_payload = asdict(result) if result is not None else None
    for key in _cache_keys_for_tenant_lookup(
        host=normalized_host,
        client_id=normalized_client_id,
        organization_id=normalized_organization_id,
...
def resolve_tenant_public_config_by_identifier(
    db: Session,
    *,
    client_id: str | None = None,
    jurisdiction_id: str | None = None,
) -> TenantPublicConfig | None:
    return resolve_tenant_public_config(db, client_id=client_id, jurisdiction_id=jurisdiction_id)


def tenant_public_config_to_dict(config: TenantPublicConfig) -> dict:
    payload = asdict(config)
    payload["market_served"] = config.market_served
    return payload
