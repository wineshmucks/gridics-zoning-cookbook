...
import { getServerBackendOrigin, joinBackendPath } from '../../lib/backend'
...
function buildBackendApiUrl(path: string): string {
  return joinBackendPath(backendOrigin, path)
}
...
export async function fetchCustomerRecord(
  ...
  try {
    ...
    const record = (await response.json()) as CustomerRecord
    ...
    const publicConfigResponse = await fetch(
      buildBackendApiUrl(`/api/public/client-config?orgid=${encodeURIComponent(organizationId)}`),
      {
        cache: 'no-store',
      },
    )
    ...
    if (publicConfigResponse.ok) {
      const publicConfig = (await publicConfigResponse.json().catch(() => null)) as
        | { logo_path?: string | null }
        | null
      if (publicConfig?.logo_path) {
        return {
          ...record,
...
export async function saveCustomerExperienceSettingsAction(
  ...
  try {
    ...
    revalidatePath(`/super-admin/${organizationId}/assistant-setup`)
    revalidatePath(`/super-admin/${organizationId}/assistant`)
...
export async function saveCustomerEmbedSettingsAction(
  ...
  try {
    ...
    revalidatePath(`/super-admin/${organizationId}`)
    revalidatePath(`/super-admin/${organizationId}/assistant-setup`)
    revalidatePath(`/super-admin/${organizationId}/assistant-embed`)
...
export async function saveCustomerGeneralSettingsAction(
  ...
  try {
    ...
    revalidatePath('/super-admin')
    revalidatePath(`/super-admin/${organizationId}`)
    revalidatePath(`/super-admin/${clerkOrganizationId}`)
    revalidatePath(`/super-admin/${organizationId}/assistant-setup`)
    revalidatePath(`/super-admin/${organizationId}/assistant`)
    revalidatePath(`/super-admin/${clerkOrganizationId}/assistant-setup`)
    revalidatePath(`/super-admin/${clerkOrganizationId}/assistant`)
    ...
    return {
      ...
      redirectPath:
        clientId !== organizationId
          ? `/super-admin/${clientId}`
          : clerkOrganizationId !== organizationId
            ? `/super-admin/${clerkOrganizationId}`
            : null,
    }
  } catch (error) {
    return {
