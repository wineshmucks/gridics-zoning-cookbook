...
if [[ "${UZONE_DEPLOY_TRACE:-0}" == "1" ]]; then
  set -x
fi
...
SKIP_BOOTSTRAP_IMPORT="${SKIP_BOOTSTRAP_IMPORT:-0}"
...
wait_for_expected_status() {
  ...
  while true; do
    local status
    local curl_args=(-s -o /tmp/uzone_smoke_response.txt -w '%{http_code}')
    if [[ "${url}" == https://* ]]; then
      curl_args=(-k "${curl_args[@]}")
    fi
    if [[ -n "${host_header}" ]]; then
      status="$(curl "${curl_args[@]}" -H "Host: ${host_header}" "${url}" || true)"
    else
...
echo "bootstrapping existing resources into terraform state"
...
if [[ "${SKIP_BOOTSTRAP_IMPORT}" == "1" ]]; then
  echo "skipping bootstrap import because SKIP_BOOTSTRAP_IMPORT=1"
else
  ...
  echo "finished bootstrapping existing resources"
fi
...
import_bucket() {
  ...
  if ! terraform state list | grep -Fxq "${address}"; then
    echo "importing existing ${address} from known bucket name ${bucket_name}"
...
echo "checking terraform state for ECR repositories"
...
echo "ecr state flags: backend=${backend_ecr_in_state} frontend=${frontend_ecr_in_state}"
...
if [[ "${SKIP_TERRAFORM_APPLY}" != "1" ]]; then
  if [[ "${backend_ecr_in_state}" == "1" && "${frontend_ecr_in_state}" == "1" ]]; then
    ...
    echo "running targeted terraform apply for core infrastructure"
    terraform apply \
      -auto-approve \
...
echo "reading terraform outputs for repository urls"
...
echo "backend_ecr_repository_url: ${BACKEND_REPO_URL}"
echo "frontend_ecr_repository_url: ${FRONTEND_REPO_URL}"
...
if [[ "${SKIP_TERRAFORM_APPLY}" != "1" ]]; then
  ...
  if [[ "${SKIP_SMOKE_TESTS}" == "1" ]]; then
    ...
    if [[ "${USE_EXISTING_ALB}" == "true" && -z "${SMOKE_TEST_BASE_URL}" ]]; then
