...
async function runMiddlewareLogic(req: NextRequest, auth?: any) {
  ...
  if (isSuperAdminRootRoute) {
    effectiveOrgId = ''
    effectiveScopePath = ''
    shouldClearOrgCookies = true
  } else if (isSuperAdminCustomerRoute && secondSegment) {
    effectiveOrgId = secondSegment
    effectiveScopePath = normalizeScopePath(`/super-admin/${secondSegment}`)
  }
  ...
  if (isAiAssistantRoute) {
    ...
    !isExcludedRoute &&
    firstSegment.startsWith('org_') &&
    firstSegment.length > 'org_'.length
  ) {
    effectiveOrgId = firstSegment
    effectiveScopePath = normalizeScopePath(`/${firstSegment}`)
