...
def test_list_assistant_conversation_reviews_compacts_large_gridics_payloads(monkeypatch) -> None:
    ...
    try:
        ...
        assert payload["gridics_call_log"][0]["response"]["received"] is True
        assert payload["gridics_call_log"][0]["response"]["status_code"] == 200
        assert payload["gridics_call_log"][0]["response"]["response_length"] == 5000
